...
export default function DepthMapGenerator() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => { setUploadedImage(event.target.result); setResultDepthMap(null); };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/generate-depth-map', {
        ...
        body: JSON.stringify({ image: uploadedImage, userId: user.uid, userEmail: user.email }),
      });
      ...
      if (data.status === 'succeeded') {
        ...
        await saveGeneration({ outsetaUid: user.uid, toolName: 'Depth Map Generator', prompt: 'Generate depth map from image', imageUrl: data.output, creditsUsed: 1 });
      } else { throw new Error('Generation failed'); }
    } catch (err) { alert(err.message || 'Error generating depth map'); }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = `depth-map-${Date.now()}.png`;
      document.body.appendChild(a); a.click(); window.URL.revokeObjectURL(url); document.body.removeChild(a);
    } catch { window.open(resultDepthMap, '_blank'); }
  };
          ...
          <div className="w-full h-full flex items-center justify-center p-4">
            <div className="relative max-w-6xl w-full">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><p className="text-xs text-white/40 mb-2 uppercase tracking-wider">Original</p><img src={uploadedImage} alt="Original" className="w-full rounded-2xl shadow-2xl" /></div>
                <div><p className="text-xs text-white/40 mb-2 uppercase tracking-wider">Depth Map</p><img src={resultDepthMap} alt="Depth Map" className="w-full rounded-2xl shadow-2xl" /></div>
              </div>
            </div>
          </div>
        ) : uploadedImage ? (
        ...
        <div className="bg-gradient-to-t from-black via-black/90 to-transparent pb-2">
          {resultDepthMap ? (
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => setResultDepthMap(null)} className="px-6 py-3 sm:px-8 sm:py-3.5 bg-white/10 hover:bg-white/20 text-white rounded-xl sm:rounded-2xl transition-all backdrop-blur-xl border border-white/10 shadow-2xl flex items-center gap-2 font-bold text-sm sm:text-base"><X size={18} /><span>Reset</span></button>
            ...
            <button onClick={handleGenerate} disabled={!uploadedImage || isGenerating || loading} className="w-full py-3 sm:py-3.5 bg-white hover:bg-gray-100 text-black font-bold text-sm sm:text-base rounded-xl sm:rounded-2xl flex items-center justify-center gap-2 sm:gap-3 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-2xl">
              {isGenerating ? <><div className="h-4 w-4 sm:h-5 sm:w-5 border-2 border-black border-t-transparent animate-spin rounded-full" /><span>Generating Depth Map...</span></> : <><Sparkles size={18} /><span>Generate Depth Map (1 Credit)</span></>}
            </button>
          )}
        </div>
      </footer>
