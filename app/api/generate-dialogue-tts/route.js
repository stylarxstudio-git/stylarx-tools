...
export async function POST(request) {
  try {
    ...
    if (useVoiceClone) {
      ...
      let uploadedAudioUrl = refAudioUrl;
      if (refAudioUrl.startsWith('data:')) {
        ...
        uploadedAudioUrl = await fal.storage.upload(blob);
      }

      result = await fal.subscribe('fal-ai/dia-tts/voice-clone', {
        input: {
          text,
          ref_audio_url: uploadedAudioUrl,
    ...
    const resultAudioUrl = result?.audio?.url || result?.data?.audio?.url;
    ...
    return NextResponse.json({ status: 'succeeded', audioUrl: resultAudioUrl });
