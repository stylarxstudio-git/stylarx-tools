...
function MaterialPreview({ albedoUrl, normalUrl, heightUrl, roughnessUrl, aoUrl }) {
  ...
  useEffect(() => {
    ...
    if (roughnessUrl) {
      loader.load(roughnessUrl, (tex) => {
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(2, 2);
        setTextures(prev => ({ ...prev, roughness: tex }));
      });
    }

    if (aoUrl) {
      loader.load(aoUrl, (tex) => {
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(2, 2);
        setTextures(prev => ({ ...prev, ao: tex }));
      });
    }
  }, [albedoUrl, normalUrl, heightUrl, roughnessUrl, aoUrl]);
...
export default function PBRGenerator() {
  ...
  const handleGenerate = async () => {
    if (!prompt.trim() || !user) {
      alert('Please enter a material description and log in first');
      return;
    }

    setIsGenerating(true);
    setGenerationProgress('Generating PBR material set...');
    ...
    try {
      ...
      if (!userCredits || userCredits.credits_remaining < creditsNeeded) {
        throw new Error(`Insufficient credits. This generation requires ${creditsNeeded} credits.`);
      }

      // ONE API CALL for all maps
      const response = await fetch('/api/generate-pbr-generator', {
        method: 'POST',
      ...
      while (prediction.status !== 'succeeded' && prediction.status !== 'failed') {
        ...
        const checkResponse = await fetch('/api/generate-pbr-generator', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (prediction.status === 'succeeded') {
        // PBR-Playground returns all maps
        const output = prediction.output;

        setResultMaps({
          albedo: output.albedo || output[0],
          normal: output.normal || output[1],
          height: output.height || output[2],
          roughness: output.roughness || output[3],
          ao: output.ao || output[4],
        });
        ...
        await deductCredits(user.uid, creditsNeeded);
        await saveGeneration({
          outsetaUid: user.uid,
          toolName: 'PBR Generator',
          prompt,
          imageUrl: output.albedo || output[0],
          creditsUsed: creditsNeeded,
        });

        setGenerationProgress('');
      } else {
        throw new Error('PBR generation failed');
      }
        ...
        {resultMaps.albedo && showPreview && mounted ? (
          <Canvas camera={{ position: [0, 0, 5], fov: 50 }}>
            <Suspense fallback={null}>
              <MaterialPreview 
                albedoUrl={resultMaps.albedo}
                normalUrl={resultMaps.normal}
                heightUrl={resultMaps.height}
                roughnessUrl={resultMaps.roughness}
                aoUrl={resultMaps.ao}
              />
            </Suspense>
          </Canvas>
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-center px-4 max-w-2xl">
              <div className="w-24 h-24 sm:w-32 sm:h-32 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-6 sm:mb-8">
                <Wand2 size={48} className="sm:w-16 sm:h-16 text-white/40" />
              </div>
          ...
          <div className="max-w-5xl mx-auto space-y-4">
            ...
            <div className="flex gap-3">
              <input 
                ...
                placeholder="Describe your material... (e.g., 'worn brick wall')" 
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    <Sparkles size={20} />
                    <span className="hidden sm:inline">{calculateCredits()} Credits</span>
