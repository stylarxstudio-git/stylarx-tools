        ...
        {mounted && (
          <Canvas shadows camera={{ position: [0, 1, 4], fov: 50 }} gl={{ antialias: true }}>
            <Suspense fallback={null}>
              ...
              {animationUrl && <AnimatedModel url={animationUrl} playing={isPlaying} />}
              <OrbitControls ref={controlsRef} enableZoom enablePan minDistance={1} maxDistance={15} target={[0, 0, 0]} makeDefault />
      ...
      <aside className="fixed top-20 sm:top-24 right-4 sm:right-6 z-50 w-72 sm:w-80 bg-white/[0.08] backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-4 sm:p-5 space-y-4 max-h-[75vh] overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        {animationUrl && (
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            ...
            <button onClick={() => setIsPlaying(!isPlaying)} className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl border border-white/10 text-sm font-bold transition-all">
