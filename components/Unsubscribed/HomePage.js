'use client';
import { Home, Wrench, HelpCircle, LogOut, LogIn } from 'lucide-react';
          ...
          <main className="p-3 sm:p-4 lg:p-6 space-y-4 max-w-full">
            ...
            <div className="relative bg-[#161618] rounded-2xl overflow-hidden p-6 sm:p-8 min-h-[140px] flex items-center justify-center">
              <div className="text-center text-white">
                ...
                <p className="text-sm mb-4 opacity-90">Upgrade to access all AI tools</p>
                <a href="https://stylarx.com/pricing" className="inline-block px-6 py-2.5 bg-white text-gray-900 font-bold rounded-lg hover:bg-gray-100 transition-all text-sm shadow-lg">Subscribe Now</a>
              </div>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 aspect-square sm:aspect-auto"><p className="text-xs text-gray-600 mb-1">Credits Used</p><h3 className="text-2xl sm:text-3xl font-bold text-gray-900">0</h3><p className="text-xs text-gray-500">This Month</p></div>
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 aspect-square sm:aspect-auto"><p className="text-xs text-gray-600 mb-1">Credits Left</p><h3 className="text-2xl sm:text-3xl font-bold text-gray-900">0</h3><p className="text-xs text-gray-500">This Month</p></div>
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 aspect-square sm:aspect-auto"><p className="text-xs text-gray-600 mb-1">Renewal</p><h3 className="text-sm sm:text-lg font-bold text-gray-900">--/--/----</h3><p className="text-xs text-gray-500">&nbsp;</p></div>
              <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 aspect-square sm:aspect-auto"><p className="text-xs text-gray-600 mb-1">Generations</p><h3 className="text-2xl sm:text-3xl font-bold text-gray-900">0</h3><p className="text-xs text-gray-500">This Month</p></div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
              <div className="lg:col-span-2 bg-white rounded-xl p-4 sm:p-6 border border-gray-200 flex items-center justify-center min-h-[150px]">
                <div className="text-center">
                  ...
                  <p className="text-sm sm:text-base text-gray-400 font-medium">Subscribe to see your top used AI</p>
                </div>
              </div>
              <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                ...
                <form onSubmit={async (e) => {
                  ...
                  try {
                    const response = await fetch('https://formspree.io/f/xqedqkzv', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ suggestion, userEmail: user?.email || 'Not logged in', userName: user?.name || 'Anonymous', _subject: 'New AI Tool Suggestion - STYLARX' }) });
                  ...
                  <button type="submit" disabled={isSubmittingSuggestion} className="mt-2 w-full py-1.5 sm:py-2 bg-gray-900 text-white rounded-lg font-medium hover:bg-gray-800 text-xs sm:text-sm">{isSubmittingSuggestion ? 'Sending...' : 'Submit'}</button>
                </form>
              </div>
            </div>
              ...
              <div className="overflow-x-auto w-full">
                <table className="w-full min-w-[600px]">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      {['Product','Date','Time','Status','Prompt','Action'].map(h => <th key={h} className="px-3 sm:px-4 py-2 text-left text-xs font-bold text-gray-600 uppercase">{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
