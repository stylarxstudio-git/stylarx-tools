'use client';
import { Home, Wrench, HelpCircle, LogOut, LogIn, Search, Lock, ChevronLeft } from 'lucide-react';
...
export default function ToolsPage() {
  ...
  const tools = [
    { id: 'gobo', name: 'Gobo Generator', description: 'Studio-quality lighting stencils.', credits: 1, thumbnail: '/Gobo-Generator-Thumbnail.jpg', path: '/gobo-generator', themeColor: '#8e8e8e' },
    { id: 'scene-stager', name: 'AI Scene Stager', description: 'Stage 3D models in photorealistic scenes.', credits: 1, thumbnail: '/Scene-Gen-Thumbnail1.jpg', path: '/scene-stager', themeColor: '#5b5d5e' },
    ...
    { id: 'image-to-hdri', name: 'Image to HDRI', description: 'Convert any image into a 360° HDRI environment.', credits: 1, thumbnail: '/Image-to-HDRI-Thumbnail.jpg', path: '/image-to-hdri', themeColor: '#008cff' },
    { id: 'depth-map', name: 'Depth Map Generator', description: 'Convert photos into 3D height maps for displacement.', credits: 1, thumbnail: '/Depth-Map-Thumbnail.jpg', path: '/depth-map', themeColor: '#4d4d4d' },
    { id: 'image-to-pbr', name: 'Image to PBR', description: 'Convert photos into complete PBR texture sets.', credits: 2, thumbnail: '/Image-to-PBR-Thumbnail.jpg', path: '/image-to-pbr', themeColor: '#939696' },
    { id: 'pbr-generator', name: 'PBR Generator', description: 'Generate complete PBR materials from text prompts.', credits: 3, thumbnail: '/PBR-Generator-Thumbnail.png', path: '/pbr-generator', themeColor: '#000000' },
        ...
        <div className="flex-1 lg:ml-60 overflow-auto pt-16 lg:pt-0">
          <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4 sticky top-0 lg:top-0 z-30">
            <div className="relative w-full max-w-sm text-black">
              ...
              <input type="text" placeholder="Search engines..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-black outline-none text-sm" />
            </div>
          </header>
          <main className="p-6 lg:p-10">
            <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredTools.map((tool) => {
                ...
                return (
                  <div key={tool.id} onClick={() => hasAccess ? router.push(tool.path) : router.push('/gated')} className="group relative aspect-square rounded-[1rem] overflow-hidden hover:shadow-2xl transition-all duration-500 cursor-pointer bg-[#121212]">
                    <div className="absolute inset-0 transition-transform duration-700 group-hover:scale-110">
                      <img src={tool.thumbnail} alt={tool.name} className="w-full h-full object-cover opacity-100 group-hover:opacity-30 transition-opacity" />
                    </div>
                    <div className="absolute top-5 left-5 right-5 flex justify-between items-center z-20">
                      {!hasAccess && (<div className="bg-black/40 backdrop-blur-md p-2 rounded-xl border border-white/10 shadow-xl"><Lock size={16} className="text-white" /></div>)}
                      <span className="ml-auto px-3 py-1 bg-white/10 backdrop-blur-md text-white text-[10px] font-black rounded-full border border-white/10 uppercase tracking-widest shadow-xl">{tool.credits} {tool.credits > 1 ? 'Credits' : 'Credit'}</span>
                    </div>
                    <div className="absolute inset-x-0 bottom-0 p-6 pt-32 flex flex-col justify-end z-10" style={{ background: `linear-gradient(to top, ${tool.themeColor} 0%, ${tool.themeColor}33 60%, transparent 100%)` }}>
                      <h3 className="font-bold text-white text-base sm:text-lg transition-transform duration-300 transform translate-y-12 group-hover:translate-y-0">{tool.name}</h3>
                      <p className="text-white/80 text-[10px] sm:text-xs mt-1 line-clamp-1 opacity-0 group-hover:opacity-100 transition-all duration-300 delay-75 transform translate-y-4 group-hover:translate-y-0">{tool.description}</p>
                      <div className="mt-4 opacity-0 group-hover:opacity-100 transition-all duration-300 delay-100 transform translate-y-4 group-hover:translate-y-0">
                        <button className="w-full py-2.5 bg-white text-black text-[10px] sm:text-xs font-black rounded-xl uppercase tracking-tighter shadow-xl hover:bg-gray-200 transition-all">{hasAccess ? 'Generate' : 'Unlock Access'}</button>
                      </div>
                    </div>
                    <div className="absolute bottom-6 left-6 group-hover:opacity-0 transition-opacity duration-300 z-0">
                      <h3 className="font-bold text-white/90 text-base sm:text-lg tracking-tight uppercase italic opacity-60">{tool.name}</h3>
                    </div>
                  </div>
                );
              })}
